...
#   SWAP%/used/total (GiB) : from /proc/meminfo; used  = SwapTotal - SwapFree
...
OUT_FILE_ARG="${3:-}"   # optional explicit output CSV path (default: results/system_monitor_<ts>.csv)
...
echo "timestamp,cpu_pct,mem_pct,mem_used_gib,mem_total_gib,swap_pct,swap_used_gib,swap_total_gib,npu_pct" > "$OUT_FILE"
...
trap 'echo ""; echo "[monitor-system] stopped, log at $OUT_FILE"; exit 0' INT TERM
...
while true; do
  ...
  ts=$(date '+%Y-%m-%d %H:%M:%S')
  echo "${ts},${cpu_pct},${mem_pct},${mem_used_gib},${mem_total_gib},${swap_pct},${swap_used_gib},${swap_total_gib},${npu_pct}" >> "$OUT_FILE"
  echo "[monitor-system] ${ts}  cpu=${cpu_pct}%  mem=${mem_pct}% (${mem_used_gib}/${mem_total_gib} GiB)  swap=${swap_pct}% (${swap_used_gib}/${swap_total_gib} GiB)  npu=${npu_pct:-N/A}%"
